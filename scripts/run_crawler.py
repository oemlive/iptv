...
def scheduled_due(cfg, now=None):
    sch=cfg.get('schedule',{})
    if not sch.get('enabled',True): return False
    try:
        if now is None:
            now=datetime.now(ZoneInfo(str(sch.get('timezone','UTC'))))
        elif now.tzinfo is None:
            now=now.replace(tzinfo=timezone.utc)
    except Exception:
        now=datetime.now(timezone.utc)
    weekdays={int(x) for x in sch.get('weekdays',list(range(7))) if str(x).isdigit()}
    times={str(x).strip()[:5] for x in sch.get('times',['02:00'])}
